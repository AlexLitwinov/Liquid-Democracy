...
#   op_i, op_j - opinions of agent i and j
...
# A_ij = (1 - |op_i - op_j|) * sigmoid(r * (pow_j - pow_i))
#
...
attractiveness_absolute <- function(op_i, op_j, pow_i, pow_j, resp) {
  (1 - abs(op_i - op_j)) / (1 + exp(-resp * (pow_j - pow_i)))
}


# Attractiveness v2 — Log-Ratio Power  [current default]
# A_ij = (1 - |op_i - op_j|) * sigmoid(r * log(pow_j / pow_i))
#
...
attractiveness_log <- function(op_i, op_j, pow_i, pow_j, resp) {
  (1 - abs(op_i - op_j)) / (1 + exp(-resp * log(pow_j / pow_i)))
}
...
selfweight_absolute_power <- function(i, nb, op, pow, responsiveness) {
...
selfweight_mean_contest <- function(i, nb, op, pow, responsiveness) {
...
selfweight_argmax <- function(i, nb, op, pow, responsiveness) {
  if (!length(nb)) return(0.5)
  a_nb   <- (1 - abs(op[i] - op[nb])) *
...
selfweight_argmax_log <- function(i, nb, op, pow, responsiveness) {
  if (!length(nb)) return(0.5)
  a_nb   <- (1 - abs(op[i] - op[nb])) *
